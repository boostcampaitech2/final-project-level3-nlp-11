...
class DenseRetrieval:
    ...
    def set_contexts(self):
        data_path = self.pickle_path
        self.contexts = []
        self.places = []
        self.area_idx = {}
        with open(f"{data_path}/pair.json", "r", encoding="utf-8-sig") as f:
    ...
    def get_embedding(self):
        ...
        if os.path.isfile(f"{emd_path}/embedding.bin"):
            with open(f"{emd_path}/embedding.bin", "rb") as file:
    ...
    def save_embedding(self, save_path: str):
        ...
        with open(f"{save_path}/embedding.bin", "wb") as file:
            pickle.dump(p_embedding, file)
        print("Embedding pickle saved.")

    def inference(
        self, single_query, topk: int = 5, area: str = "전국", use_elastic=True
    ):
        """
